...
def inference_encoder():
    ...
    return Model(inputs=enc_input,outputs=[enc_lstm_output,enc_last_hidden_state,enc_last_cell_state])
...
def inference_decoder():
    ...
    return Model(inputs=[dec_input,another_dec_input,dec_initial_hidden_state,dec_initial_cell_state],
               outputs=[dec_output,dec_last_hidden_state,dec_last_cell_state])


def generate_molecule(input_data):

    inf_enc = inference_encoder()
    states = inf_enc.predict(input_data)
    enc_output = states[0]
    states.pop(0)
    gen_sequence = np.array([[config.INPUT_VOCABULARY.index("<")]])
    ...
    while not stop_generation:
        ...
        gen_sequence = np.array([[nxt_gen_char_idx]])
